...
# Генерируем уникальный ID запуска сервера. Если сервер рестартнет на Render, клиенты это поймут и сбросят стейт.
...
def init_db():
    ...
    for col, dtype in columns_to_add.items():
        try:
            ...
            pass
    # Создаем таблицу событий (Source of Truth для всех realtime действий)
    cursor.execute("""
...
# --- Global Event Log (Source of Truth in SQLite) ---
async def push_event(event_type: str, data: dict) -> dict:
    """Единая точка входа для всех broadcast-событий.
    Записывает событие в SQLite (гарантия порядка) и рассылает клиентам."""
    conn = sqlite3.connect(DB_PATH)
    conn.execute("PRAGMA journal_mode=WAL")
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO events (event_type, data, ts) VALUES (?, ?, ?)",
    ...
    await manager.broadcast({"event": event_type, "data": data, "_seq": event_id})
    return entry
...
async def get_events(last_id: int = 0, token: str = ""):
    ...
    try:
        ...
        conn.execute("PRAGMA journal_mode=WAL")
        cursor = conn.cursor()
        cursor.execute("SELECT id, event_type, data, ts FROM events WHERE id > ? ORDER BY id ASC", (last_id,))
        rows = cursor.fetchall()
        conn.close()
...
async def websocket_endpoint(websocket: WebSocket, token: str):
    try:
        ...
        await websocket.send_json({"event": "init", "data": {"server_run_id": SERVER_RUN_ID}})
        try:
            while True:
