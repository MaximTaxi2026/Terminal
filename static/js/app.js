...
function setupWebSocket() {
    ...
    wsClient.onopen = () => {
        ...
        // Catch up on missed or active messages when connected or reconnected
        if (isBroadcast) {
            loadHistory((msg) => {
...
function initGuestPage() {
    ...
    function initDrawCanvas() {
        if (!drawCtx) {
            ...
            const getCoords = (e) => {
                ...
                return {
                    cssX: (clientX - rect.left) * (drawCanvas.offsetWidth / rect.width),
                    cssY: (clientY - rect.top) * (drawCanvas.offsetHeight / rect.height),
            ...
            const moveDraw = (e) => {
                ...
                drawCtx.lineTo(coords.cssX, coords.cssY);
    ...
    sendBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (response.ok) {
                showNotification('ОЖИДАНИЕ ПОДТВЕРЖДЕНИЯ СЕРВЕРА...');
                messageInput.value = '';
                clearPhoto();
...
// --- Drawing System (Broadcast) ---
let broadcastCtx = null;
let broadcastDrawings = new Map(); // track by client_id
...
function handleBroadcastDraw(event, data) {
    ...
    if (event === 'draw_clear') {
        if (canvas && broadcastCtx) {
            broadcastCtx.clearRect(0, 0, canvas.width, canvas.height);
        }
        broadcastDrawings.clear();
        return;
    }
    ...
    if (event === 'draw_start') {
        broadcastDrawings.set(client_id, { lastX: px, lastY: py });
    } else if (event === 'draw_move') {
        const state = broadcastDrawings.get(client_id);
        if (state) {
            broadcastCtx.beginPath();
            broadcastCtx.moveTo(state.lastX, state.lastY);
            broadcastCtx.lineTo(px, py);
            broadcastCtx.strokeStyle = color;
            broadcastCtx.lineWidth = 4;
            broadcastCtx.shadowBlur = 10;
            broadcastCtx.shadowColor = color;
            broadcastCtx.stroke();
        ...
        broadcastDrawings.delete(client_id);
    }
}
