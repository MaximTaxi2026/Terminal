    ...
    wsClient.onopen = () => {
        ...
        heartbeatInterval = setInterval(() => {
            if (wsClient && wsClient.readyState === WebSocket.OPEN) {
                ...
                // If no pong received in 5 seconds, drop connection to trigger reconnect
        ...
        }, 15000); // 15 seconds is optimal for Render
    };
...
let currentMessageTimeout1 = null;
let currentMessageTimeout2 = null;
let currentDisplayingMessageId = null;
...
function processBroadcastQueue() {
    if (isMessageDisplaying || broadcastQueue.length === 0) return;
    
    isMessageDisplaying = true;
    const msg = broadcastQueue.shift();
    currentDisplayingMessageId = msg.id;
    
    const feed = document.getElementById('feed');
    if (!feed) {
        isMessageDisplaying = false;
        currentDisplayingMessageId = null;
        return;
    }
    ...
    try {
        ...
        currentMessageTimeout1 = setTimeout(() => {
            card.classList.add('hidden-card');
            currentMessageTimeout2 = setTimeout(() => {
                card.remove();
                isMessageDisplaying = false;
                currentDisplayingMessageId = null;
        ...
        console.error('Error processing broadcast queue:', e);
        isMessageDisplaying = false;
        currentDisplayingMessageId = null;
        setTimeout(processBroadcastQueue, 100);
    }
}
...
function removeMessageFromDOM(id) {
    if (typeof broadcastQueue !== 'undefined') {
        broadcastQueue = broadcastQueue.filter(msg => msg.id !== id);
    }
    if (currentDisplayingMessageId === id) {
        if (currentMessageTimeout1) clearTimeout(currentMessageTimeout1);
        if (currentMessageTimeout2) clearTimeout(currentMessageTimeout2);
        isMessageDisplaying = false;
        currentDisplayingMessageId = null;
        setTimeout(processBroadcastQueue, 50);
    }
