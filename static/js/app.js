...
let lastSeq = 0;  // Порядковый номер последнего полученного события (для replay)
...
function setupWebSocket() {
    ...
    wsClient.onopen = () => {
        ...
        // Восстановление состояния при подключении/переподключении
        if (isBroadcast) {
            if (lastSeq > 0) {
                // Переподключение: догружаем пропущенные события через event log
                fetch(`/api/events?last_id=${lastSeq}&token=${authToken}`)
                    .then(r => r.json())
                    .then(events => {
                        console.log(`[REPLAY] Replaying ${events.length} missed events since seq=${lastSeq}`);
                        events.forEach(entry => {
                            if (entry.seq > lastSeq) lastSeq = entry.seq;
                            handleWebSocketEvent({ event: entry.event, data: entry.data, _seq: entry.seq });
                        });
                    })
                    .catch(e => console.error('[REPLAY] Failed to replay events:', e));
            } else {
                // Первое подключение: загружаем историю сообщений как обычно
                loadHistory((msg) => {
