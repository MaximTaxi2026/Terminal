...
async function setupAuth() {
    ...
    if (authToken) {
        // Strict server-side verification to prevent bypass
        ...
        try {
            const response = await fetch(`/api/verify?token=${authToken}`);
            if (response.ok) {
                unlock();
                return;
            } else {
